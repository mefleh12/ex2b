...
// TO START THE SERVER WRITE IN THE TERMINAL: node server.js
...
// API route to return movie data as JSON
    ...
    db.get(sql, [filmCode], (err, film) => {
        ...
        db.all(reviewSql, [film.FilmCode], (err, reviews) => {
            ...
            db.all(detailsSql, [film.FilmCode], (err, details) => {
                ...
                // Send everything as JSON (not render)
                res.json({ film, reviews, filmDetails });
            });
        });
    });
});
